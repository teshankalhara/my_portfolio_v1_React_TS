            ...
            <div className="container lg:grid lg:grid-cols-2 lg:items-stretch">
                <div className="mb-12 lg:mb-0 lg:flex lg:flex-col">
                    ...
                    <div className="flex items-center gap-2 mt-auto reveal-up">
                        {socialLinks && socialLinks.map((item, key) => {
                            return (
                                <a href={item.href} target="_blank" className="w-12 h-12 grid place-items-center ring-inset ring-2 ring-zinc-50/5 rounded-lg transition-[background-color,color] hover:bg-zinc-50 hover:text-zinc-950 active:bg-zinc-50/80 reveal-up" key={key}>
                                    {item.icon}
                                </a>
                            )
                        })}
                ...
                <form action="https://getform.io/f/bjjjerjb" method="POST" className="xl:pl-10 2xl:pl-20">
                    <div className="md:grid md:items-center md:grid-rows-2 md:gap-2">
                        <div className="mb-4">
                            ...
                            <input type="text" name="name" id="name" className="text-field reveal-up" autoComplete="name" placeholder="Teshan Kalhara" required />
                        </div>
                        <div className="mb-4">
                            <label htmlFor="email" className="label reveal-up">
                                Email
                            </label>
                            <input type="email" name="email" id="email" className="text-field reveal-up" autoComplete="email" placeholder="teshan@email.com" required />
                        </div>
                        <div className="mb-4">
                            <label htmlFor="message" className="label reveal-up">
                                Message
                            </label>
                            <textarea name="message" id="message" placeholder="Hii" required className="text-field resize-y min-h-32 max-h-80 reveal-up">
                                {/*comment*/}
                            </textarea>
                        </div>
